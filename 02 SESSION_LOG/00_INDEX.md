...
| [2026-07-16_2115_local-tree-repair-and-docs-relocation.md](2026-07-16_2115_local-tree-repair-and-docs-relocation.md) | Claude (Sonnet) | Fixed `getSupabaseServerClient is not a function` crash: restored local code tree to match git HEAD, then found and fixed genuine corruption committed in git itself — `lib/db/index.ts` had literal `\n`/`\"` escape sequences baked into the text from partway through the file to EOF. `npx tsc --noEmit` now clean project-wide; user confirmed localhost:3000 running. Also relocated canonical docs to `01 DOCS/`/`02 SESSION_LOG/` (copied content, nothing deleted) and updated CLAUDE.md's file-location rules to match. Fix not yet committed to git — see session file's Open Items. |
