...
export async function assemblePracticeSession(
  ...
  // Composition tier per non-due skill (PRD F2's illustrative "review /
  // functions / inference / mixed" breakdown, generalized to real
  // domain/skill names rather than hardcoded category labels — see
  // 09 WIKI/DEV/SESSION_ASSEMBLER.md). Due skills are always 'review'; the
  // more urgent half of the remaining (non-due) skills are 'priority', the
  // rest 'mixed'. This reuses the same ranking selection already computed
  // above rather than a parallel budgeting pass, so composition can never
  // drift from what was actually selected.
  const nonDueOrdered = skillPriorities.filter((p) => !p.isDue); // already vulnerabilityScore-ascending
  ...
  const questionCache = new Map<string, Question[]>(); // skillId -> candidate pool, best calibration match first

  async function candidatesFor(priority: SkillPriority): Promise<Question[]> {
    const cached = questionCache.get(priority.skill.id);
    if (cached) return cached;

    const questions = await fetchQuestionsBySkill(supabase, priority.skill.id, 10);
  ...
  // 6. Confidence-builder reserve pool (PRD F2): the highest-mastery skills
  // this student has (p_mastery >= 0.75), easiest validated question each,
  // not already used above. Held out of `items` — SessionRunner splices one
  // in after 2 consecutive misses. Naturally empty for a brand-new student
  // with no mastery history yet, which is the correct degrade (no
  // confidence-builder available rather than a fabricated one).
